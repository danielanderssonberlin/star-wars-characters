  ...
  try {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <Link 
          href="/" 
          className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-400 transition-colors mb-4 group"
        >
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-gray-800">
          <div>
            <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter text-foreground mb-2">
              {person.name}
            </h2>
            <div className="flex flex-wrap gap-4 text-gray-400 text-sm">
              <span className="flex items-center gap-1.5"><User className="size-4 text-blue-600" /> {person.gender}</span>
              <span className="flex items-center gap-1.5"><Globe className="size-4 text-blue-600" /> {homeworld.name}</span>
            </div>
          </div>
          <div className="px-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded text-blue-500 text-xs font-bold uppercase tracking-widest">
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {films.map((film, i) => (
              <div key={i} className="p-4 bg-gray-900/10 border border-gray-800 rounded-lg flex items-center justify-between group hover:border-blue-500/20 transition-colors">
                <div>
                  <h4 className="font-semibold text-foreground group-hover:text-blue-400 transition-colors">{film.title}</h4>
